...
from .abisolar import init, query_params
...
DEFAULT_NAME = "Abisolar inverter"
        ...
        **kwargs,
        ...
        while True:
            try:
                ...
                _LOGGER.info("Serial device %s %s connected", device, baudrate)
                while True:
                    try:
                        _LOGGER.info("Updating device")
                        params = await query_params()
                        _LOGGER.debug("Received: %s", json.dumps(params, indent=4))
